...
read.csv(file = 'data/contribution.csv') %>%
  ...
  # Group the remaining data by class year, and sum our new LifetimeGiving object within each year
  group_by(Class.Year) %>%
  summarise(ClassLifetimeGiving = sum(LifetimeGiving)) %>%
  # Format as dollars
  mutate(ClassLifetimeGiving = scales::dollar(ClassLifetimeGiving))
```

Note that a few lines of code can be run at a time to check that the results are as expected. Also, we'd just need to enter the new filename at the top to get updated results each week.

# Merging datasets

Another common Excel task is merging columns from different files, for example by VLOOKUP. This is very easy (and significantly faster for large files!) in R.
